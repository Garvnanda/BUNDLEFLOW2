...
function convertToSerializableArgument(argument) {
  ...
  if ("data" in argument) {
    return import_ts_sdk.AccountAddress.from(argument.data).toString();
  }
...
function normalizePayloadForIC(payload, aptosConfig) {
  ...
  if ("bytecode" in payload) {
    return (0, import_ts_sdk.generateTransactionPayload)(payload);
  }
  if (aptosConfig) {
    return payload.abi !== void 0 ? (0, import_ts_sdk.generateTransactionPayloadWithABI)({ ...payload, abi: payload.abi }) : (0, import_ts_sdk.generateTransactionPayload)({ aptosConfig, ...payload });
  }
  ...
  return "multisigAddress" in payload ? {
    multisig_address: import_ts_sdk.AccountAddress.from(payload.multisigAddress).toString(),
  ...
  async getConnectedAccounts() {
    ...
    return Object.values(pairings).map(
      ({ accountAddress, accountEd25519PublicKeyB64, accountPublicKeyB64 }) => ({
        address: import_ts_sdk2.AccountAddress.from(accountAddress),
...
var ACDappClient = class {
  constructor({
    ...
    this.transport = new import_web_transport.WebWalletTransport(frontendBaseURL, provider, tgWebAppURL);
    this.dappId = dappId;
  ...
  async isIcAccount(address) {
    ...
    return icAccounts.find((account) => account.address.equals(import_ts_sdk3.AccountAddress.from(address))) !== void 0;
  }
  async getConnectedAccounts() {
  ...
  async disconnect(address) {
    if (await this.isIcAccount(address)) {
      const stringAddress = import_ts_sdk3.AccountAddress.from(address).toString();
  ...
  async connect({
    ...
    const serializedRequest = import_wallet_api3.ConnectRequest.serialize(this.dappInfo, requestArgs);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    ...
    if (response.args.status === "approved") {
      ...
      return (0, import_wallet_api3.makeUserApproval)({ account });
    }
    return response.args;
  }
  async signIn(args) {
    const serializedRequest = import_wallet_api3.SignInRequest.serialize(this.dappInfo, args);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_wallet_api3.SignInResponse.deserialize(serializedResponse);
    return response.args;
  }
  async signMessage(args) {
    ...
    if (await this.isIcAccount(signerAddress)) {
      const { chainId } = args;
      const network = (0, import_wallet_api3.chainIdToNetwork)(chainId);
      let message;
      let nonce;
      try {
      ...
      const signatureBytes = import_ts_sdk3.Hex.fromHexInput(hexSignature).toUint8Array();
      const signature = signatureBytes.length === import_ts_sdk3.Ed25519Signature.LENGTH ? new import_ts_sdk3.Ed25519Signature(signatureBytes) : import_ts_sdk3.AnySignature.deserialize(new import_ts_sdk3.Deserializer(signatureBytes));
      return (0, import_wallet_api3.makeUserApproval)({
        fullMessage,
        signature
      });
    }
  ...
  async signAndSubmitTransaction(args) {
    ...
    if (await this.isIcAccount(signerAddress)) {
      ...
      if (network === import_wallet_api2.shelbynet.name) {
        throw new Error("Shelbynet not supported");
      }
      const aptosConfig = new import_ts_sdk3.AptosConfig({ network });
      let convertedArgs;
      if (feePayer !== void 0) {
        const payload = await normalizePayloadForIC(args.payload, aptosConfig);
        const rawTxn = await (0, import_ts_sdk3.generateRawTransaction)({
          aptosConfig,
        ...
        convertedArgs = {
          ...
          rawTxn: new import_ts_sdk3.FeePayerRawTransaction(rawTxn, [], feePayer.address)
        };
      } else {
      ...
      return (0, import_wallet_api3.makeUserApproval)({ txnHash: hash });
    }
...
var ACKeylessClient = class {
  constructor({
    ...
    this.transport = new import_web_transport2.WebWalletTransport(frontendBaseURL, provider);
  }
  // region Public API
  async isConnected() {
    const serializedRequest = import_wallet_api4.IsConnectedRequest.serialize(this.dappInfo);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_wallet_api4.IsConnectedResponse.deserialize(serializedResponse);
    return response.args;
  }
  async getConnectedAccounts() {
    const serializedRequest = import_wallet_api4.GetConnectedAccountsRequest.serialize(this.dappInfo);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_wallet_api4.GetConnectedAccountsResponse.deserialize(serializedResponse);
    return response.args;
  }
  async disconnect() {
    const serializedRequest = import_wallet_api4.DisconnectRequest.serialize(this.dappInfo);
    await this.transport.sendRequest(serializedRequest);
  }
  async connect() {
    const serializedRequest = import_wallet_api4.ConnectRequest.serialize(this.dappInfo);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_wallet_api4.ConnectResponse.deserialize(serializedResponse);
    return response.args;
  }
  async signMessage(args) {
    const serializedRequest = import_wallet_api4.SignMessageRequest.serialize(this.dappInfo, args);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_wallet_api4.SignMessageResponse.deserialize(serializedResponse);
    return response.args;
  }
  async signTransaction(args) {
    const normalizedArgs = "transaction" in args ? import_wallet_api4.SignTransactionRequest.normalizeArgs(args) : args;
    const serializedRequest = import_wallet_api4.SignTransactionRequest.serialize(this.dappInfo, normalizedArgs);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_wallet_api4.SignTransactionResponse.deserialize(serializedResponse);
    return response.args;
  }
  async signAndSubmitTransaction(args) {
    const serializedRequest = import_wallet_api4.SignAndSubmitTransactionRequest.serialize(this.dappInfo, args);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_wallet_api4.SignAndSubmitTransactionResponse.deserialize(serializedResponse);
    return response.args;
  }
  // endregion
};
